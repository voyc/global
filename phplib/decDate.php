...
function convertStringToDecimalDate( $s) {
	...
	if ($d == -1 || $d == '') {
		return convertDifficultStringToDecimalDate($str);
	}
	else {
...
function convertDifficultStringToDecimalDate($str)
{
	/** precision code */ $UNDEFINED   =    0;
	/** precision code */ $YEAR        =    1;

	// handle formats of the type 3,000,000
	//
	$str = str_replace( ",", "", $str );

	// is the date a BC date?
	//
	$bc = false;
	if ( preg_match( '( BC)', $str ) == 1 )
	{
		// remember that it's a BC date, remove the 'BC' from the string
		//
		$bc = true;
		$str = str_replace( " BC", "", $str );
	}
	...
	if ( count( $words ) == 1 )
	{
		// special case, just one word in the string, assume it's a year
		//
		return array( 'decidate' => ($bc ? '-' : '') . $str, 'precision' => $YEAR );
	}

	// Look for the first word that is definitely a year: an all-numerals word
	// greater than 31 (can't be a month or a day or an hour, so that's 
	// pretty safe)
	//
	foreach ( $words as $w )
	{
		if ( ( preg_match( '(\d+)', $w ) == 1 ) && ( $w > 31 ) )
		{
			// current word ($w) is probably a year
			// Now try the unix time conversion again, replacing the actual year
			// with a known-supported year, then once we have the result,
			// replace the integer portion with the actual year.  Make sure to get
			// the fraction right if this year is a leap year (or not a leap year)
			//
			$tempYear = isLeapYear( $w ) ? '1964' : '1970';
			$tempStr = str_replace( $w, $tempYear, $str );
		        $my_d = strtotime( $tempStr );
			$unixTime = convertUnixTimestampToDecimalDate( $my_d );
			$unixTime[ 'decidate' ] = str_replace( $tempYear, $w,
						               $unixTime[ 'decidate' ] );
			if ( $unixTime[ 'precision' ] != $UNDEFINED )
				return $unixTime; // convertUnixTimeStamp worked!

			// test conversion failed, let's check if date is of form <y> <m> <d>
			// 
		 	if ( ( count( $words ) == 3 ) && ( $w == $words[0] ) )
		 	{
				// yes, there are 3 words, and the "year" that we found
				// is the first of them
				//
				$tempStr_3 = $tempYear . '-' . $words[1] . '-' . $words[2];
				$my_d = strtotime( $tempStr_3 );
				$unixTime = convertUnixTimestampToDecimalDate( $my_d );
				$unixTime[ 'decidate' ] = str_replace( $tempYear, $w, 
								       $unixTime[ 'decidate' ] );
			}
	...
	// couldn't find a successful converison
	//
        return array( 'decidate' => 0, 'precision' => $UNDEFINED );
}
