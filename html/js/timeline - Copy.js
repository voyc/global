...
voyc.Timeline = function(e) {
	...
	this.ctxRuler = null;
	this.divRowContainer = null;
	this.divStreamContainer = null;
	this.divVertical = [];
	this.divRulerTime = [];
	...
	this.tickHeight = 3;
	this.superHeight = 6;
	this.streamHeight = 20;
	this.streamSpacer = 2;
	this.maxRowHeight = 20;
	this.maxRowTextHeight = 10;
	this.rowFontsizeNone = '0pt';
	this.rowFontsizeNormal = '8pt';
	this.rowFontsizeHovered = '12pt';
	this.streamFontsizeNormal = '12pt';
	...
	// rows are in the background, variable height, depending on the number of rows
	this.rows = [];
	this.numRows = 0;
	this.rowHeight = 1;
	this.rowSpacer = 0;
	this.rowFontsize = this.rowFontsizeNormal;

	// streams are on top, fixed height, number limited by height of container
	this.streams = [];
	this.lastUsedStreamNum = -1;
	this.numStreams = 0;  // set by resize, depending on height
...
voyc.Timeline.prototype.create = function(bounds) {
	...
	// create the scroller, contains both ruler and stream container
	var e = document.createElement('div');
	e.id = 'tline-scroller';
	...
	e = document.createElement('div');
	e.id = 'tline-rowcontainer';
	e.style.position = 'relative';
	var ht = (this.div.offsetHeight-this.rulerHeight-this.zoombarHeight);
	e.style.height = ht + 'px';
	this.divScroller.appendChild(e);
	this.divRowContainer = e;

	// create the stream container
	e = document.createElement('div');
	e.id = 'tline-streamcontainer';
	e.style.position = 'relative';
	e.style.height = ht + 'px';
	e.style.top = (0-ht) + 'px';
	this.divScroller.appendChild(e);
	this.divStreamContainer = e;

	// create streams and verticals. Can be changed on resize.
	this.createStreams();
	this.createVerticals();
	this.maxDots = Math.floor(this.numStreams * (this.divStreamContainer.offsetWidth / this.minPixelsPerDotWidth));
...
/**
 * Create the streams.
 * Called by resize.  Number of streams depends on height of container.
**/
voyc.Timeline.prototype.createStreams = function() {
	var oldNumStreams = this.numStreams;
	this.numStreams = Math.floor((this.divStreamContainer.offsetHeight) / (this.streamHeight+this.streamSpacer));
	if (this.numStreams > oldNumStreams) {
		for (var i=oldNumStreams; i<this.numStreams; i++) {
			var d = document.createElement('div');
			d.id = 'stream_'+i;
			d.className = 'stream';
			d.style.height = this.streamHeight + 'px';
			d.style.marginTop = this.streamSpacer + 'px';
			this.divStreamContainer.appendChild(d);
			this.streams[i] = {
				div:d,
				forebear:0,
				bars:[],
	...
	else if (this.numStreams < oldNumStreams) {
		for (var i=this.numStreams; i<oldNumStreams; i++) {
			this.divStreamContainer.removeChild(this.streams[i].div);
...
voyc.Timeline.prototype.createVerticals = function() {
	...
	this.numVerticals = Math.floor(this.divStreamContainer.offsetWidth / this.minPixelsPerPeriod);
...
voyc.Timeline.prototype.setupDragging = function() {
	// scroller contains ruler and stream container
...
voyc.Timeline.prototype.resize = function() {
	// reset height of stream container
	//this.divStreamContainer.style.height = (this.div.offsetHeight-this.rulerHeight-this.zoombarHeight) + 'px';

	var newHt = (this.div.offsetHeight-this.rulerHeight-this.zoombarHeight);
	this.divRowContainer.style.height = newHt + 'px';
	this.divStreamContainer.style.height = newHt + 'px';
	this.divStreamContainer.style.top = (0-newHt) + 'px';
	...
	// create additional streams and verticals, only if necessary
	this.createStreams();
	this.createVerticals();
	this.maxDots = Math.floor(this.numStreams * (this.divStreamContainer.offsetWidth / this.minPixelsPerDotWidth));
...
voyc.Timeline.prototype.drawData = function() {
	...
	for (var i in this.data) {
		record = this.data[i];
		bar = record.ts;
		if (record.q && bar.layer == 'row' && bar.rowNum < 0) {
	...
	for (var o in datasortedforward) {
		...
		if (bar.rowNum < 0 && record.f) {
			this.assignBarToRow(record,'forebear');
		}
	}
	...
	for (var o in datasortedreverse) {
		...
		bar = record.ts;
		if (bar.rowNum < 0) {
			this.assignBarToRow(record,'reverse');
		}
	}
	...
	for (var o in datasortedforward) {
		...
		bar = record.ts;
		if (bar.rowNum < 0) {
			this.assignBarToRow(record,'forward');
		}
	}
	...
	// remove excessive timebars from streams
...
voyc.Timeline.prototype.finishRows = function() {
	...
	for (var i=0; i<this.numRows; i++) {
		...
		row.vertPosition = -1;
		if (row.showing) {
			row.vertPosition = this.numRowsShowing;
			this.numRowsShowing++;
		}
	}

	// interleave: merge forebear rows where possible without overlap
	this.numRowsVertical = this.numRowsShowing;
	for (var i=0; i<this.numRows; i++) {
		row = this.rows[i];
	...
	for (var i=0; i<this.numRows; i++) {
		rowi = this.rows[i];
		if (rowi.showing) {
			for (var j=i+1; j<this.numRows-1; j++) {
				rowj = this.rows[j];
				if (rowj.showing) {
	...
	// combine bars with identical headlines
	for (var i=0; i<this.numRows; i++) {
		row = this.rows[i];
		var phead = '';
		for (var n in row.bars) {
			bar = row.bars[n];
			// first bar, make overflow visible and inc index
			if (bar.record.h != phead) {
				bar.headpos = 'first';
				phead = bar.record.h;
				voyc.addClass(bar.div,'headfirst');
				bar.zindex++;
				bar.div.style.zIndex = bar.zindex;
			}
			// other bars, set fontsize 0 and left border dotted
			else {
				// make the left border a dotted line
				bar.headpos = 'last';
				voyc.addClass(bar.div,'headlast');
			}
		}
	}

	// set top and height of each bar
	this.rowHeight = Math.floor(this.divRowContainer.offsetHeight / this.numRowsVertical);
	this.rowHeight = Math.min(this.rowHeight, this.maxRowHeight);
	this.rowFontsize = this.rowFontsizeNormal;
	if (this.rowHeight < this.maxRowTextHeight) {
		this.rowFontsize = this.rowFontsizeNone;
	}
	var bar;
	for (var i=0; i<this.numRows; i++) {
		row = this.rows[i];
		for (var n in row.bars) {
			bar = row.bars[n];
			bar.div.style.height = this.rowHeight + 'px';
			bar.top = (this.rowHeight*row.vertPosition);
			bar.div.style.top = bar.top + 'px';
			bar.fontSize = (bar.headpos == 'first') ? this.rowFontsize : this.rowFontsizeNone;
			bar.div.style.fontSize = bar.fontSize;
		}
	}
}

voyc.Timeline.prototype.assignBarToRow = function(record,direction) {
	var bar = record.ts;
	var n = -1;
	...
	if (direction == 'forebear') {
		if (record.f) {
			for (var i=0; i<this.numRows; i++) {
				if (this.rows[i].forebear == record.f) {
					n = i;
					break;
				}
			}
	...
	// set the occupied positions on this row
	if (n >= 0) {
		this.rows[n].forebear = record.f;
		if (this.rows[n].begin > record.b) {
			this.rows[n].begin = record.b;
		}
		if (this.rows[n].end < record.e) {
			this.rows[n].end = record.e;
		}
		bar.rowNum = n;
		this.rows[n].bars.push(bar);
	}
	return n;
}

voyc.Timeline.prototype.assignBarToStream = function(record) {
	var bar = record.ts;
	var n = -1;
	// find a stream with the same forebear
	if (record.f) {
		for (var i=0; i<this.numStreams; i++) {
			if (this.streams[i].forebear == record.f) {
				n = i;
				break;
			}
		}
	...
	// choose the next lower stream	
	var n = this.lastUsedStreamNum;
	n = ((n+1) < this.numStreams) ? n+1 : 0;
	var failsafe = 0;
	while (this.streams[n].forebear) {
		n = ((n+1) < this.numStreams) ? n+1 : 0;
		failsafe++;
		if (failsafe > this.numStreams) {
			voyc.debug.alert('failsafe break encountered');
			break; 
		}
	}
	this.lastUsedStreamNum = n;

/*
	// 2. find the first stream that has room on the end
	if (n < 0) {
		for (var i=0; i<this.numStreams; i++) {
			if (this.streams[i].end < record.b) {
				n = i;
				break;
			}
		}
...
*/
	// set the occupied positions on this stream
	if (n >= 0) {
		this.streams[n].forebear = record.f;
		if (this.streams[n].begin > record.b) {
			this.streams[n].begin = record.b;
		}
		if (this.streams[n].end < record.e) {
			this.streams[n].end = record.e + this.yearsFromPixels(this.minPixelsPerDotWidth);
		}
	}
	return n;
}
...
voyc.Timeline.prototype.xassignBarToStream = function(record) {
	// if possible, find a stream with the same forebear
	for (var i=0; i<this.numStreams; i++) {
		if (this.streams[i].forebear == record.f) {
			return i;
		}
	...
	// if two dots overlap, kick out the one with the lower magnitude

	// this means we have to loop thru the records on this stream
	// or only if the current record overlaps the stream
	// go and find the most recent record added
	// to find two dots to compare

//	if (this.streams[n].begin > record.b) {
//		this.streams[n].begin = record.b;


	// a comparison of begin and end dates applies only to spans
	
	// dots must compare by pixels
...
/**
loop thru all bars on a stream
...
voyc.Timeline.prototype.disqualifyOverlaps = function() {
	...
	var stream;
	var x,y,w,t;
	for (var n=0; n<this.numStreams; n++) {
		stream = this.streams[n];
